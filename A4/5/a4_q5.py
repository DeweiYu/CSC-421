...
for c in classes:
    ...
    for cap_shape in cap_shapes:
        # Calculate the likelihood of cap_shape given the class
        if cap_shape in class_data["cap-shape"].values:
            likelihoods[c][cap_shape] = class_data["cap-shape"].value_counts(normalize=True).get(cap_shape, 0)
        else:
            likelihoods[c][cap_shape] = 0.0
...
for a in cap_shapes:     
    ...
    for c in classes:
        posteriors[a][c] = (
            likelihoods[c][a] * priors[c] / marginals[a]
            if marginals[a] > 0 else 0.0
        )
